...
1. ✅ **Remove credentials from URL embedding** - Fixed security vulnerability where passwords were embedded in URLs (line 758). Now shows a warning and uses capture button instead.
2. ✅ **Add configuration management** - Created `config.py` to centralize all configuration values. Supports environment variables for sensitive data.
3. ✅ **Improve database connection handling** - Added context manager (`get_db_connection()`) to ensure proper connection cleanup and transaction management.
4. ✅ **Add input validation** - Created `utils/validators.py` with validation functions for URLs, IP addresses, roll numbers, emails, and names. Integrated into ESP32-CAM URL input.
